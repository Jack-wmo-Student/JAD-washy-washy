...
//            }
	...
	private boolean validateAdminAccess(HttpServletRequest request, HttpServletResponse response) throws IOException {
	...
	private void sendJsonResponse(HttpServletResponse response, int status, String message) throws IOException {
		...
		String jsonResponse = String.format("{\"message\": \"%s\"}", message.replace("\"", "\\\"") // Escape quotes in
																									// message
		);
